...
export default function InvoiceBuilderPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const updated = await api.patch<any>(`/invoices/${id}`, {
        line_items: lineItems, tax_rate: taxRate, discount, currency, due_date: dueDate, notes,
      });
      setInv(updated);
      toast({ title: 'Saved' });
    } catch (e: any) {
  ...
  const send = async () => {
    await save();
    const updated = await api.patch<any>(`/invoices/${id}/send`);
    setInv(updated);
    toast({ title: 'Marked as sent' });
  };

  const markPaid = async () => {
    const updated = await api.patch<any>(`/invoices/${id}/paid`);
    setInv(updated);
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {inv.status !== 'paid' && inv.status !== 'cancelled' && (
              <>
                ...
                {inv.status === 'sent' && <Button variant="outline" size="sm" className="text-green-700" onClick={markPaid}><CheckCircle className="w-3.5 h-3.5 mr-1.5" />Mark paid</Button>}
              </>
            )}
            <Button size="sm" onClick={save} disabled={saving}><Save className="w-3.5 h-3.5 mr-1.5" />{saving ? 'Saving…' : 'Save'}</Button>
          </div>
        }
      />
      <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input type="number" value={taxRate} onChange={(e) => setTaxRate(+e.target.value)} className="h-8" min={0} max={100} />
          </div>
        </div>
        ...
        <div className="border rounded-xl overflow-hidden">
          ...
          <div className="px-4 py-2 border-t">
            <Button variant="ghost" size="sm" className="text-xs" onClick={() => setLineItems((p) => [...p, newItem()])}>
          ...
          <div className="w-64 space-y-2 text-sm">
            ...
            {discount > 0 && (
              <div className="flex justify-between items-center">
                ...
                <Input type="number" value={discount} onChange={(e) => setDiscount(+e.target.value)} className="h-6 w-24 text-right text-xs" />
              </div>
            )}
            {discount === 0 && (
              <button className="text-xs text-muted-foreground hover:text-foreground" onClick={() => setDiscount(1)}>+ Add discount</button>
            )}
