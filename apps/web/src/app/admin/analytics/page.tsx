...
import { BarChart2, MessageSquare, CheckCheck, Eye, Reply, XCircle, Download } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { api } from '@/lib/api';
import type { PlatformAnalytics } from '@whatslark/shared';
import {
...
const RANGE_DAYS: Record<Exclude<Range, 'custom'>, number> = { '7d': 7, '30d': 30, '3m': 90 };

function exportCsv(data: DayData[]) {
...
export default function AdminAnalyticsPage() {
  ...
  const [messages, setMessages] = useState<PlatformAnalytics['messages']>([]);
  const [campaigns, setCampaigns] = useState<PlatformAnalytics['campaigns']>([]);
  ...
  useEffect(() => {
    ...
    api.get<PlatformAnalytics>(`/admin/analytics?${params.toString()}`)
      .then((res) => {
  ...
  const days = range === 'custom'
    ? (from && to ? Math.max(1, Math.round((new Date(to).getTime() - new Date(from).getTime()) / (24 * 60 * 60 * 1000)) + 1) : 30)
    : RANGE_DAYS[range];
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Platform-wide performance across all companies</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => exportCsv(chartData)}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {(['7d', '30d', '3m', 'custom'] as Range[]).map((r) => (
          <Button
            key={r}
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-6 mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {[
              { label: 'Total Messages', value: overview.total, sub: `Last ${days} days`, icon: MessageSquare, color: 'text-blue-600' },
